...
// Servidor simples que serve arquivos estáticos e fornece API para listar desenhos
...
// Função para processar requisição POST
...
const server = http.createServer((req, res) => {
    ...
    // Endpoint da API para listar desenhos
    if (pathname === '/api/drawings') {
    ...
    // Endpoint da API para gerar desenho customizado
    if (pathname === '/api/generate-drawing') {
        console.log('  → Endpoint /api/generate-drawing encontrado');
        
        // Verificar método HTTP
        if (req.method !== 'POST') {
            console.log(`  ✗ Método ${req.method} não permitido. Esperado: POST`);
            res.writeHead(405, { 
        ...
        console.log('  → Método POST confirmado, processando dados...');
        ...
        parsePostData(req)
            .then(async (data) => {
                ...
                if (!data.theme || !data.theme.trim()) {
                    ...
                    res.end(JSON.stringify({ error: 'Tema não fornecido' }));
                    return;
                }
                ...
                if (!apiKey || apiKey === 'sua-chave-aqui') {
                    ...
                    res.end(JSON.stringify({ error: 'OPENAI_API_KEY não configurada. Por favor, edite o arquivo config.js e adicione sua chave da API.' }));
                    return;
                }
